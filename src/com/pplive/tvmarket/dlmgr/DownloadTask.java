...
public class DownloadTask extends AsyncTask<Void, Integer, Long> {
    ...
    public DownloadTask(Context context, DownloadTaskData data, String path, DownloadTaskListener listener){
        super();
        mData= data;
        mListener = listener;
    ...
    protected void onPreExecute() {
        ...
        mData.status = DownloadTaskData.PREPARE;
        if (mListener != null)
    ...
    protected void onProgressUpdate(Integer... progress) {
        ...
        if (progress.length > 1) {
            ...
            mDownloadPercent = (mDownloadSize + mPreviousFileSize) * 100 / mData.totalSize;
    ...
    protected void onPostExecute(Long result) {
        ...
        if (result == -1 || mInterrupt || mError != null) {
            ...
            mData.status = DownloadTaskData.DOWNLOADFAIL;
            if (mListener != null) {
                mListener.onError(this, mError);
            }
            return;
        ...
        // finish download
        mData.status = DownloadTaskData.DOWNLOADED;
    ...
    private long download() throws NetworkErrorException, IOException, FileAlreadyExistException,
        ...
        if (DownloadManagerConfig.DEBUG) {
            Log.e(TAG, "--" + getTitle() + "----begin download-----totalSize: " + mData.totalSize);
        }
        ...
        if (mFile.exists() && mData.totalSize == mFile.length()) {
            if (DownloadManagerConfig.DEBUG) {
                Log.e(TAG, "--" + getTitle() + "---Output file already exists. Skipping download.");
            }
            ...
            if (DownloadManagerConfig.DEBUG) {
                Log.e(TAG, "--" + getTitle() + "---File is not complete, download now.");
                Log.e(TAG, "--" + getTitle() + "---File length:" + mTempFile.length() + " totalSize:" + mData.totalSize);
            }
        }
        ...
        if (DownloadManagerConfig.DEBUG) {
			Log.e(TAG, "--" + getTitle() + "---storage:" + storage + " totalSize:" + mData.totalSize);
        }
        ...
        if ((mPreviousFileSize + bytesCopied) != mData.totalSize && mData.totalSize != -1 && !mInterrupt) {
            throw new IOException("Download incomplete: " + bytesCopied + " != " + mData.totalSize);
        }
    ...
    public int copy(InputStream input, RandomAccessFile out) throws IOException,
        ...
        if (DownloadManagerConfig.DEBUG) {
            Log.e(TAG, "--" + getTitle() + "---begin copy----length" + out.length());
        }
        ...
        try {
            ...
            while (!mInterrupt) {
                ...
                count += n;
//                if (DownloadManagerConfig.DEBUG) {
//                    Log.e(TAG, "--" + getTitle() + "---copying----count=" + count);
//                }
                /*
                 * check network
                 */
